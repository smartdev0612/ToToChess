...
class ChargeController extends WebServiceController 
{
	...
	function listAction()
	{
		...
		$totalRes = $partnerModel->getRecommendChargeTotal($partnerSn, $beginDate, $endDate);
		$total = $totalRes["cnt"];
		$total_amount = $totalRes["sum_amount"];

		$pageMaker	= $this->displayPage($total, 100, $page_act);
		$list 			= $partnerModel->getRecommendChargeList($partnerSn, $beginDate, $endDate, $where, $pageMaker->first, $pageMaker->listNum);
