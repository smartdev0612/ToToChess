...
class MemberController extends WebServiceController 
{
	...
	function listAction()
	{
		...
		$total = $model->getPartnerInMemberTotalTop($id, $where);
		$pageMaker = $this->displayPage($total, 100);
		$list = $model->getPartnerInMemberListTop($id, $where, $pageMaker->first, $pageMaker->listNum);		
	...
	function partner_listAction() {
		...
		$this->view->define("content","content/member/partner_list.html");
		...
		if($keyword!="")
		{
			...
			else if($field=="rec_name")
			{
				$where=" and rec_name like '%".$keyword."%'";
			}
			else if($field=="rec_bankusername")
			{
				$where=" and rec_bankusername like '%".$keyword."%'";
			}
		}
		
		$total = $model->getMyPartnerTotal($this->auth->getId(), $where);
		$pageMaker = $this->displayPage($total, 100);
		$list = $model->getMyPartner($this->auth->getId(), $where, $pageMaker->first, $pageMaker->listNum);		
...
*/
	...
	function popup_joinAction()
	{
		...
		$this->view->define("content","content/member/popup.join.html");
		$partnerModel = $this->getModel("PartnerModel");		
		$act	= $this->request('act');
		
		$parentInfo = $partnerModel->getPartnerById($this->auth->getId());

		if($act=="add")
		{
			$topRecId = $this->auth->getId();
			$uid = $this->request('uid');
			$passwd = $this->request('passwd');
			$name = $this->request('name');
			$phone = $this->request('phone');
	...
	function popup_detailsAction()
	{
		...
		$parentInfo = $model->getPartnerById($this->auth->getId());
		...
		if($act=="add")
		{
			...
			$model->modifyMemberDetails($where, $memo, $rec_lev, $rec_name, $rec_bankname, $rec_bankusername, $rec_banknum, $rec_email, $rec_phone, $tex_type, $tex_rate_sport, $tex_rate_minigame, $rec_one_folder_flag, $urlidx, $tex_get_member_id, $rec_parent_id);
	...
	function partner_structAction()
    {
        ...
        $list 			= $model->getPartnerList3($rec_id, $beginDate, $endDate);
    ...
    function popup_mem_listAction()
    {
        ...
        } else if($rec_top_id != null && $rec_top_id != ''){
            $list = $model->getPartnerInMemberListTop($rec_top_id);
        }
    ...
    function popup_betAction()
    {
        ...
        $page_act = "rec_sn=".$rec_sn."&rec_top_id=".$rec_top_id."&is_sport=".$is_sport."&select_keyword=".$selectKeyword."&keyword=".$keyword."&begin_date=".$begin_date."&end_date=".$end_date;

        $total = $model->getMemberTotalCartCntByPartner($rec_sn, $rec_top_id, $is_sport, $begin_date, $end_date, $where);  // 전체 카트내역
        $pageMaker	= $this->displayPage($total, 10, $page_act);
        $list = $model->getMemberBetListByPartner($rec_sn, $rec_top_id, $is_sport, $pageMaker->first, $pageMaker->listNum, $begin_date, $end_date, '', $where);
