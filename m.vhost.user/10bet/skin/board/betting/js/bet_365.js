...
BetList.prototype.ClearAll = function() {
	for (var i = 0; i<this._items.length; i++) {
		...
		$j(item_no+" .homeTeam").removeClass("menuOff_up");
		$j(item_no+" .bet").removeClass("menuOff_up");
		$j(item_no+" .awayTeam").removeClass("menuOff_up");
		$j(item_no+" .muSelect").removeClass("menuOff_up");
...
function del_all_cookies(){
	var table_list = ['a10','a30','a40','a50','a70','a90','a91','a92','a912','a93','a94','a95','a96','a97','a98'];
	...
	$j.each(data, function() {
		...
		//{{{ 그외 =========================================
			...
			if (is_start) {
				...
				"</div>"+
				"<div class='title_area'>";
				if(VarBoTable == 'a91' || VarBoTable == 'a912'){
					board_view_txt = board_view_txt + 
				...
				"</span>"+
				"</div>"+
				"<div id='box_detail_"+this.bn_id+"_"+this.group_id+"' class='bet_box box_col_"+this.bn_id+"_"+this.group_id+"'>";
			...
			if (is_start) {
				...
				if(i == (bonus_cnt_arr.length)) {
					board_view_txt2 = board_view_txt2 + "<div id='col_"+this.bn_id+"_"+this.group_id+"' style='width:100%;margin-left:0%;text-align:center;'>";
				}else {
					board_view_txt2 = board_view_txt2 + "<div id='col_"+this.bn_id+"_"+this.group_id+"' style='width:100%;margin-left:0%;text-align:center;display:none;'>";
				}
			...
			if (is_ing) {
				...
				var onclick_str = " onclick=\"showAndHideElemId(this, 'detail_"+this.bn_id+"_"+this.group_id+"',"+game_id[0]+");\" ";
				var count_str = "+"+(this.group_cnt-3);
				...
				var btn_count = "<input type='button' class='side_bet box_detail_"+this.bn_id+"_"+this.group_id+"' style='cursor:pointer;background:none;border:0;' value='"+count_str+"'" + onclick_str + "><input type='hidden' id='hidden_col_"+this.bn_id+"_"+this.group_id+"' value='"+count_str+"'>";
				var btn_count2 = "<button class='button_close' value='"+count_str+"'" + onclick_str +"></button>";

				if(tmp_group_cnt <= 3) {
					if(this.wr_7 == '승무패' && (VarBoTable == 'a91' || VarBoTable == 'a912')){
						board_view_txt = board_view_txt + 
							"<div class='colum01' style='width:100%;'>"+
							"<div class='box01'>"+
							"<div class='btn_area'>"+
							"<button id=\"chk_"+this.wr_id+"_1\" class='btn_bet01 "+menuOffCss+"' "+chk1+">"+this.wr_1+"</button>"+
							"</div>"+
							"<div class='btn_area'>"+
							"<button id=\"chk_"+this.wr_id+"_3\" class='btn_bet01 "+menuOffCss+"' "+chk3+">"+this.handy+"</button>"+
							"</div>"+
							"<div class='btn_area'>"+
							"<button id=\"chk_"+this.wr_id+"_2\" class='btn_bet01 "+menuOffCss+"' "+chk2+">"+this.wr_2+"</button>"+
							"</div>"+
							"</div>"+
							"</div>";
					}
					else if(this.wr_7 == '승무패' && VarBoTable == 'a92'){
						board_view_txt = board_view_txt + 
							"<div class='colum01'>"+
							"<div class='box01'>"+
							"<div class='btn_area' style='width:100%;'>"+
							"<button id=\"chk_"+this.wr_id+"_1\" class='btn_bet01 "+menuOffCss+"' "+chk1+">"+home_name[0]+" ["+this.wr_1+"]</button>"+
							"</div>"+
							"</div>"+
							"</div>";
					}
					/*
						...
						if(tmp_group_cnt == 4) {
							...
							""+btn_count+""+
							"<!-- 상세 배팅 -->"+
							"<div class='detail_bet' id='detail_"+this.bn_id+"_"+this.group_id+"' style='display:none;'>"+
							"<!-- 버튼 영역 -->"+
							"<div class='btn_box'>"+
...
function showAndHideElemId(e, eid, game_id) {
	...
	if (d3 == 'block'){   d3 = 'none';     /*$j(e).val('접기');*/	}
	else if(d3 == 'none')            {   d3 = 'block'; /*$j(e).val($j('#hidden_'+eid).val());*/}
	
	$j('#'+eid).css('display', d);
	
	$j('#box_'+eid).css('display', d2);
	
	$j('.box_'+eid).css('display', d3);
}
