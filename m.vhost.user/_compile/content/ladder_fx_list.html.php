...
<div id="sub_menu_game_end" style="height:auto; margin-bottom:0px;">
    <ul>
        ...
        for($i=1; $i<6;$i++)
        {
            if($i == $min)
            {
                //echo "<a href='#' class='fx_on' style='margin-left: 15px'>".$i."분</a>";
                echo '<li class="sub_menu_game_end_o">
                        <a href="#" class="sub_menu_game_end_o_text">'.$i.'분</a>
                      </li>';
            } else {
                //echo "<a href='/game_list?game=fx&min=".$i."' class='fx_type' style='margin-left: 15px'>".$i."분</a>";
                echo '<li class="sub_menu_game_end">
                        <a href="/game_list?game=fx&min='.$i.'" class="sub_menu_game_end_text">'.$i.'분</a>
                      </li>';
            }        }
        ?>
    </ul>
...
<div class="ladder_wrap" style="background: url(/img/m/bg_ladder.png) repeat">
	<div class="ladder_top" style="left:50%; margin-left:-143px; height:270px;">
		<div class="ladder_area" style="width:809px; top:-10px !important; left:-93px; ">
			<iframe id="game_frame" src="http://fxgame.com/fxgame/<?php echo $min?>min/index.php" frameborder="0" border="0" scrolling="no" style="width:100%; height:700px;"></iframe>
		</div>
	</div>
	...
	<div class="ladder_cnt">
		...
		<div class="ladder_fx">
			...
			<div class="ladder_1st">
				<h4><span>1게임</span><strong  style="font-size: 13px">홀/짝</strong></h4>
				<div>
					<ul>
						<li><input type="button" value="<?php echo $fx_oe?>" class="b_odd" onclick="gameSelect('odd','<?php echo $fx_oe?>');" id="gameSt_odd"></li>
						<li><input type="button" value="<?php echo $fx_oe?>" class="b_even" onclick="gameSelect('even','<?php echo $fx_oe?>');" id="gameSt_even"></li>
					</ul>
				</div>
			</div>
			<div class="ladder_2nd">
				<h4><span>2게임</span><strong style="font-size: 13px">언더/오버</strong></h4>
				<div>
					<ul>
                        <li><input type="button" value="<?php echo $fx_uo?>" class="b_under" onclick="gameSelect('under','<?php echo $fx_uo?>');" id="gameSt_under"></li>
                        <li><input type="button" value="<?php echo $fx_uo?>" class="b_over" onclick="gameSelect('over','<?php echo $fx_uo?>');" id="gameSt_over"></li>
					</ul>
				</div>
			</div>
			<div class="ladder_3rd">
				<h4><span>3게임</span><strong  style="font-size: 13px">매수/매도</strong></h4>
				<div>
					<ul>
                        <li><input type="button" value="<?php echo $fx_bs?>" class="b_buy" onclick="gameSelect('buy','<?php echo $fx_bs?>');" id="gameSt_buy"></li>
                        <li><input type="button" value="<?php echo $fx_bs?>" class="b_sell" onclick="gameSelect('sell','<?php echo $fx_bs?>');" id="gameSt_sell"></li>
					</ul>
				</div>
			</div>
		</div>
			...
			<div class="cart_info">
				<ul>
					<li>
						<div>
							<span>게임분류</span>
							<strong>[FX <?php echo $min?>분]</strong>
						</div>
					</li>
					<li id="selBet">
						<div>
		...
		<div class="btn_area">
            ...
            <?php } else {?>
                <div class="bet_disable" style="display: block;text-align: left;font-size: 14px;height: 300px;padding-top: 20px;padding-left: 20px;padding-right: 20px">
                    FX게임 데이터제공 시간 안내 <br>
                    FX게임에서 제공하는 FX Margin 거래 데이터는 국내 증권사인 하나금융투자 API데이터를 기반으로 제공되고 있습니다.<br>
                    FX Margin 거래는 한국표준시간을 기준으로 주5일 24시간 거래가 이루어지지만 원활한 데이터제공을 위해서  FX게임에서는 <br>
                    아래같은 기준으로 데이터가 제공됩니다.<br>
                    데이터 제공시간 : 07:20 ~ 06:20 (토요일 05:00 까지)<br>
                    주말(토,일) : 휴장<br>
                </div>
            <?php } ?>
			<ul>
...
<script type="text/javascript" src="/scripts/bet/fx.js?v=<?=time();?>"></script>
<script>
