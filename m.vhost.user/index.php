...
{
    header("Location: http://son6b.com");
    die();
}
...
//$http_host = str_replace(":8080", "", $http_host);
...
//-> 움직임 없이 900초 로그아웃 처리. (위 PHP 설정값 작동안됨)
if ( count((array)$_SESSION["member"]) > 0 ) {
	if ( !$_SESSION["sessionLifeTime"] ) {
		$_SESSION["sessionLifeTime"] = time()+900;
	} else if ( $_SESSION["sessionLifeTime"] < time() ) {
		session_destroy();		
		header("Location:/");
	}	else {
		$_SESSION["sessionLifeTime"] = time()+900;
