	...
	function realTime() {
		...
		if ( secTemp <= limit_time && secTemp >= (120 - 15) ) {
	...
	function gameSelect(type, rate) {
		...
		for ( i = 0 ; i < $("input[id^='gameSt_']").length ; i++ ) {
			try {
				$("input[id^='gameSt_']").eq(i).removeClass('sadari_focus');
			} catch(e){};
		}

		if ( selectGameType != type ) {
			$("input[id=gameSt_"+type+"]").addClass('sadari_focus');
			selectGameType = type;
			selectGameRate = rate;
	...
	function gameBetting() {
		...
		var postData = {"btGameName":"2dari", "btGameTh":gameTh, "btMoney":bet_money, "btGameType":selectGameType};
