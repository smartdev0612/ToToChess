	...
	function gameSelect(type, rate) {
		if ( selectGameType != type ) {
			$("#gameSt_"+saveGameType).removeClass('focus_'+saveGameType);
			$("input[id=gameSt_"+type+"]").addClass('focus_'+type);
			selectGameType = type;
			selectGameRate = rate;
			$("#stGameRate").html(rate);
			$("#stGameIcon").html("<img src='/images/mybet_"+type+".png' style='vertical-align: middle;'>");
			saveGameType = type;
		} else {
			selectGameType = null;
			selectGameRate = 0;
			$("#stGameRate").html("0.00");
			$("#stGameIcon").html("선택하세요");
			$("#gameSt_"+type).removeClass('focus_'+saveGameType);
		}
		calHitMoney();
	}
