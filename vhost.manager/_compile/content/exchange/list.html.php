<?php /* Template_ 2.2.3 2014/01/07 17:55:08 D:\www\vhost.manager\_template\content\exchange\list.html */
...
<?php if($TPL_list_1){foreach($TPL_VAR["list"] as $TPL_V1){?>
		<tr class="link_lan" style="padding-left:1px;"  onMouseOver="this.style.backgroundColor='#e0eafe';" onMouseOut="this.style.backgroundColor=''">
			...
			<td><b><?php echo $TPL_V1["bank"]?></b></td>
			<td>
<?php if($TPL_V1["bank_count"]>0){?><a href="javascript:open_window('/member/popup_bank_account?member_sn=<?php echo $TPL_V1["member_sn"]?>',800,350)"><b><font color='red'><?php echo $TPL_V1["bank_account"]?></font></b></a>
<?php }else{?><?php echo $TPL_V1["bank_account"]?>

<?php }?>
			</td>
			<td><b><?php echo $TPL_V1["bank_owner"]?></b></td>
			<td><?php echo $TPL_V1["recommendId"]?></td>
			<td><?php if($TPL_V1["state"]==0){?>신청<?php }elseif($TPL_V1["state"]==3){?><font color=blue>대기</font><?php }?></td>
			<td class="btn">
<?php if($TPL_V1["state"]==0){?>
					<!--
					<a href="javascript:comfire_ok('','/exchange/delprocess?sn=<?php echo $TPL_V1["sn"]?>')">
