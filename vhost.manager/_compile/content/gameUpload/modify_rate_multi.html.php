...
<script>document.title = '게임등록-배당수정';</script>
<script>
	...
	function onChangeRate(ckboxSn) {
		$("#ckbox_"+ckboxSn).attr('checked','checktd');
		$("#ckboxBack_"+ckboxSn).attr('checked','checktd');
	}
...
<body>
	<div id="route">
		<h5>관리자 시스템 - 배당수정 목록</h5>
	</div>
	<h3>항목 보기</h3>
    <div id="betting_2">
        <ul id="tab">
            <li><a href="/gameUpload/modify_rate" id="sport_1">스포츠 I</a></li>
            <li><a href="/gameUpload/modify_multi_rate" id="sport_2">스포츠 II</a></li>
        </ul>
    </div>
	...
	<div id="search">
		<div class="wrap2">
			...
			<form name=frmSrh method=post action="/gameUpload/modify_multi_rate"> 
				...
				<select name="filterState">
					<option value="" <?php if($TPL_VAR["filterState"]==""){?> selected <?php }?>>대기+발매</option>
					<option value="3" <?php if($TPL_VAR["filterState"]=="3"){?> selected <?php }?>>대기중</option>
					<option value="2" <?php if($TPL_VAR["filterState"]=="2"){?> selected <?php }?>>발매중</option>
				</select>
...
<?php }}?>
	...
	<form id="form1" name="form1" method="post" action="?">
		...
		<input type="hidden" name="filterState" value=<?php echo $TPL_VAR["filterState"]?>>
		...
		<input type="hidden" name="parsing_type" value=<?php echo $TPL_VAR["parsing_type"]?>>
...
<?php
	if ( $TPL_list_1 ) { 
		...
		foreach ( $TPL_VAR["list"] as $TPL_V1 ) {
			$TPL_I1++;
			if ( $TPL_V1["user_view_flag"] == 0 ) $addTrStyle = "notView";
			else $addTrStyle = "";
?>
					<tr class="<?=$addTrStyle;?>">
						<td><input id="ckbox_<?php echo $TPL_V1["sn"]?>" name="y_id[]" type="checkbox" value="<?php echo $TPL_V1["sn"]?>" onClick="select_to(this);"/></td>
...
<?php }?>
						...
						<td><input type="text" id="home_rate" name="home_rate[<?php echo $TPL_V1["sn"]?>]" size="5" value="<?php echo $TPL_V1["home_rate"]?>" style="border:1px #97ADCE solid;" onKeyUp="onChangeRate('<?php echo $TPL_V1["sn"]?>');" onFocus="$(this).css('background-color','#CEFBC9');" onFocusOut="$(this).css('background-color','#FFD8D8');"><br><?php echo number_format($TPL_V1["home_total_betting"],0)?></td>
						<td><input type="text" id="draw_rate" name="draw_rate[<?php echo $TPL_V1["sn"]?>]" size="5" value="<?php echo $TPL_V1["draw_rate"]?>" style="border:1px #97ADCE solid;" onKeyUp="onChangeRate('<?php echo $TPL_V1["sn"]?>');" onFocus="$(this).css('background-color','#CEFBC9');" onFocusOut="$(this).css('background-color','#FFD8D8');"><br><?php echo number_format($TPL_V1["draw_total_betting"],0)?></td>
						<td><input type="text" id="away_rate" name="away_rate[<?php echo $TPL_V1["sn"]?>]" size="5" value="<?php echo $TPL_V1["away_rate"]?>" style="border:1px #97ADCE solid;" onKeyUp="onChangeRate('<?php echo $TPL_V1["sn"]?>');" onFocus="$(this).css('background-color','#CEFBC9');" onFocusOut="$(this).css('background-color','#FFD8D8');"><br><?php echo number_format($TPL_V1["away_total_betting"],0)?></td>
<?php if($TPL_VAR["state"]!=3){?>
...
<?php }?>
						<td><input id="ckboxBack_<?php echo $TPL_V1["sn"]?>" name="y_id_back[]" type="checkbox" value="<?php echo $TPL_V1["sn"]?>" onClick="select_to(this);"/></td>
					</tr>
<?php }}?>
			</tbody>
		</table>
