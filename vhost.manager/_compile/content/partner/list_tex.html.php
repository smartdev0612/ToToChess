...
<script>document.title = '총판관리-총판정산';</script>
<script>
function confirmTex() {
    ...
    $.post("/partner/texSettle", param, function(result) {
        if( $.trim(result) == "true" ) {
            alert("총판정산이 완료되였습니다.");
            $('#texFormSubmit').trigger('click');
        }
    });
}
</script>
	...
	<div id="search">
		<div class="wrap">
			<form action="?" method="GET" name="texForm" id="texForm">
				...
				<!-- 검색버튼 -->
				<input name="texFormSubmit" id="texFormSubmit" type="image" src="/img/btn_search.gif" class="imgType" />
...
<?php 
	...
	$sum_tex_money += $TPL_V1["tex_money"];
	$sum_get_tex_money += $TPL_V1["get_tex_money"];
	$sum_betting_to_ready += $TPL_V1["betting_to_ready"];
?>
			<tr>
				<td>
                    <?php if(isset($TPL_V1["rec_sn"])) {?>
                        <a href="javascript:open_window('/partner/memberDetails?idx=<?php echo $TPL_V1["rec_sn"]?>',640,440)">
				...
				<td><?php echo $TPL_V1["save_rate"]?> %</td>
				<td><?php echo number_format($TPL_V1["tex_money"],0)?></td>
				<td><?php echo number_format($TPL_V1["get_tex_money"],0)?></td>
				<td>
					<?php 
