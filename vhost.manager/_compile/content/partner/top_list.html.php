...
<script>document.title = '★부본사관리-부본사목록';</script>
<script>
	function go_del(url)
	{
...
<div class="wrap">
	...
	<div id="route">
		<h5>★ 관리자시스템 > 부본사 관리 > <b>부본사 목록</b></h5>
	</div>
...
?>
			<tr>
				<td><a href="javascript:open_window('/partner/memberDetailsTop?idx=<?php echo $TPL_V1["Idx"]?>',640,440)"><?php echo $TPL_V1["rec_id"]?></a></td>
...
<?php }?>&nbsp;<a href="javascript:void(0)" onclick="go_del('?act=del&id=<?php echo $TPL_V1["rec_id"]?>');return false;"><img src="/img/btn_s_del.gif" title="삭제"></a>&nbsp;
			...
			<tr id="d_<?php echo $TPL_V1["Idx"]?>" style="display:none;" class="gameDetail">
				<td align="center" colspan="13">
					<table cellspacing="1" id="d_<?php echo $TPL_V1["Idx"]?>" width="100%">
						<tr bgcolor="#ade8a0">				  
							<th>소속총판</th>
							<th>ID</th>
							<th>닉네임</th>
...
<?php if($TPL_item_2){foreach($TPL_V1["item"] as $TPL_V2){?>														
						<tr bgcolor="#ede8e8">
							<td><?php echo $TPL_V2["recommend_uid"]?></td>
...
<?php }}?>
	...
	<div id="wrap_btn">
  	<input type="submit" name="del_Submit" value="부본사 등록" class="Qishi_submit_a" onmouseover="this.className='Qishi_submit_b'"  onmouseout="this.className='Qishi_submit_a'" onclick="javascript:window.open('/partner/top_popup_join','memo','width=650,height=350')"/>
  </div>
