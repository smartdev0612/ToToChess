...
<script>
    ...
    function popupBet($rec_top_id, $rec_sn, $is_sport)
    {
        ...
        if($rec_top_id != '')
        {
            url = '/member/popup_bet?rec_top_id='+$rec_top_id;
        } else if($rec_sn != '') {
            url = '/member/popup_bet?rec_sn='+$rec_sn;
        }

        url += "&is_sport="+$is_sport;
...
?>
			<tr>
				...
				<td>
                    <a href="#" onclick="window.open('/member/popup_mem_list?rec_top_id=<?php echo $TPL_V1["rec_id"]?>','','resizable=yes scrollbars=yes top=5 left=5 width=1600 height=650')"><?php echo $TPL_V1["member_count"]?></a>
                </td>
				...
				<td>
                    <a href="#" onclick="javascript:popupBet('<?php echo $TPL_V1["rec_id"]?>', '', 1)"><?php echo number_format($TPL_V1["total_betting_sport"],0)?></a>
                    / <a href="#" onclick="javascript:popupBet('<?php echo $TPL_V1["rec_id"]?>', '', 0)"><?php echo number_format($TPL_V1["total_betting_mini"],0)?></a>
				</td>
                <td>
                    <?php
                    if($TPL_V1['rec_tex_type'] == 'Swin_Mbet') {
...
<?php if($TPL_item_2){
    ...
    ?>
        <tr>
            ...
            <td>
                <a href="javascript:open_window('/member/popup_details?idx=<?php echo $TPL_V2["Idx"];?>',660,460)"><?php echo $TPL_V2["rec_id"]?></a>
            </td>
            <td><?php echo $TPL_V2["rec_name"]?></td>
            <td>
                <a href="#" onclick="window.open('/member/popup_mem_list?rec_sn=<?php echo $TPL_V2["Idx"]?>','','resizable=yes scrollbars=yes top=5 left=5 width=1600 height=650')"><?php echo $TPL_V2["member_count"]?></a>
            </td>
            <!--<td onclick="toggle('d_<?php /*echo $TPL_V1["Idx"]*/?>')"><?php /*echo $TPL_V1["charge_count"]*/?></td>-->
            <td><?php echo $tex_type;?></td>
            <td>
            ...
            <td><?php echo number_format($TPL_V2["charge_sum"],0)?></td>
            <td><?php echo number_format($TPL_V2["exchange_sum"],0)?></td>
            <td><?php echo number_format($TPL_V2["charge_sum"] - $TPL_V2["exchange_sum"],0)?></td>
            <td>
                <a href="#" onclick="javascript:popupBet('', '<?php echo $TPL_V2["Idx"]?>', 1)"><?php echo number_format($TPL_V2["total_betting_sport"],0)?></a>
                / <a href="#" onclick="javascript:popupBet('', '<?php echo $TPL_V2["Idx"]?>', 0)"><?php echo number_format($TPL_V2["total_betting_mini"],0)?></a>
            </td>
            <td>
                <?php
                    if($TPL_V2['rec_tex_type'] == 'Swin_Mbet') {
