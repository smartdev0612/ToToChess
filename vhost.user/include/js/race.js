	...
	function gameSelect(type, rate) {
		//-> 선택된 이미지 복원
		for ( i = 0 ; i < jq$("img[id^='gameSt_']").length ; i++ ) {
			try {
				jq$("img[id^='gameSt_']").eq(i).attr('src',jq$("img[id^='gameSt_']").eq(i).attr("src").replace("_u.","_d."));
			} catch(e){};
		}

		if ( selectGameType != type ) {
			jq$("img[id=gameSt_"+type+"]").attr('src',jq$("img[id=gameSt_"+type+"]").attr("src").replace("_d.","_u."));
			selectGameType = type;
			selectGameRate = rate;
			jq$("#stGameRate").html(rate);
			jq$("#stGameTitle").html(gameTitleList[type]);
		} else {
			selectGameType = null;
			selectGameRate = 0;
			jq$("#stGameRate").html("0.00");
			jq$("#stGameTitle").html("선택하세요");
		}
		calHitMoney();
	}
	...
	function gameBetting() {
		if ( Number(VarMaxBet) == 0 ) {
			swal("유출픽 및 내부사정으로 배팅이 일시적 중단되었습니다.");
			return;
		}
