var PowerBallBoard = function (ready_round, game_type) {
    ...
    this.get_result = function () {
        self.ajax_object = $.ajax({
            ...
            success: function (response) {
                if (self.ready_round == response.times) {
                    self.getNewResultfetchError = false;
                    self.getNewResult = true;
                    self.result = response;
                } else if (self.ready_round != response.times) {
    ...
    this.animate_callback = function () {
        self.$result_board.find('.round').html("이번 " + self.result.times + "회차");
        ...
        setTimeout(function () {
            self.$waiting_board.removeClass("msg").find(".tx").html('<span id="countdown_clock" class="countdown_clock">-분 --초 후</span>  <strong class="next_turn"></strong>회차 추첨 시작');
            self.ready_round += 1;
